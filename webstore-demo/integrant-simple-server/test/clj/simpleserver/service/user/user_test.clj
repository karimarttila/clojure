...
(deftest initial-users-db-test
  ...
  (testing "Initial usersdb count"
    (let [initial-len (count (ss-user-s/-get-users (ss-tc/test-env)))]
      (is (= initial-len 3)))))

(deftest email-already-exists?-test
  (log/debug "ENTER email-already-exists?-test")
  (testing "Existing email should return true"
    (let [ret (ss-user-s/email-already-exists? (ss-tc/test-env) "kari.karttinen@foo.com")]
      (is (= ret true))))
...
(deftest add-new-user-test
  ...
  (testing "Adding a new non-existing user, should succeed"
    (let [ret (ss-user-s/add-new-user (ss-tc/test-env) "jamppa.tuominen@foo.com" "Jamppa" "Tuominen" "passw0rd")]
