...
class MyTableImporter:
    ...
    def get_table(self, my_aws_profile, my_prefix, my_env, my_table, my_host):
        self.debug("ENTER - " + "get_table")
        session = boto3.Session(profile_name=my_aws_profile)
        if my_aws_profile == 'local-dynamodb':
            dynamodb = session.resource(service_name='dynamodb', endpoint_url='http://' + my_host + ':8000')
        else:
    ...
    def import_product_groups(self, my_aws_profile, my_prefix, my_env, my_table, my_csv_file, my_host):
        self.debug("ENTER - " + "import_product_groups")
        table = self.get_table(my_aws_profile, my_prefix, my_env, my_table, my_host)
    ...
    def import_products(self, my_aws_profile, my_prefix, my_env, my_table, my_csv_file, my_host):
    ...
    def import_users(self, my_aws_profile, my_prefix, my_env, my_table, my_csv_file, my_host):
...
def import_csv(my_aws_profile, my_prefix, my_env, my_table, my_csv_file, my_host):
    ...
    if my_table == 'product-group':
        ret = importer.import_product_groups(my_aws_profile, my_prefix, my_env, my_table, my_csv_file, my_host)
    elif my_table == 'product':
        ret = importer.import_products(my_aws_profile, my_prefix, my_env, my_table, my_csv_file, my_host)
    elif my_table == 'users':
        ret = importer.import_users(my_aws_profile, my_prefix, my_env, my_table, my_csv_file, my_host)
...
if __name__ == '__main__':
    if len(sys.argv) != 7:
        print("Usage: python3 table_importer.sh <aws-profile> <prefix> <env> <table> <csv file> <host>")
        exit(-1)
    ...
    my_host = sys.argv[6]
    import_csv(my_aws_profile, my_prefix, my_env, my_table, my_csv_file, my_host)
