...
In that previous Replicant exercise I used the [Gadget extension](https://github.com/cjohansen/gadget-inspector) to visualize my data in the Chrome developer tools. In this new exercise I used [Dataspex](https://github.com/cjohansen/dataspex). Dataspex is essentially Gadget 2.0 implementation.
...
Make a http post with the parameters.
...
Navigated to the new product page: clear the new product form and validation errors (if coming from another page), and register the `:page/navigated` and `page/previous` (as with all route actions).
...
Short instructions how to start the development environment. For more information read blog post [Calva Multi-REPL](TODO).
...
In Calva, connect to backend and frontend REPLs:

- Backend REPL: In VSCode/Calva: Command Palette => `Calva: Connect to Running REPL Server in the Project` => Choose: `deps.edn`
- Frontend REPL: In VSCode/Calva: Command Palette => `Calva: Connect to Running REPL Server in the Project` => Choose: `shadow-cljs` => `Select which build to connect to` => Choose: `:app`
