...
try{if(construct)if(construct=function(){throw Error();},Object.defineProperty(construct.prototype,"props",{set:function(){throw Error();}}),"object"===typeof Reflect&&Reflect.construct){try{Reflect.construct(construct,[])}catch(x){var control=x}Reflect.construct(fn,[],construct)}else{try{construct.call()}catch(x$588){control=x$588}fn.call(construct.prototype)}else{try{throw Error();}catch(x$589){control=x$589}fn()}}catch(sample){if(sample&&control&&"string"===typeof sample.stack){for(var sampleLines=
...
callback(!0);while(null!==callback)}});syncQueue=null}catch(error$592){throw null!==syncQueue&&(syncQueue=syncQueue.slice(i+1)),Scheduler_scheduleCallback(Scheduler_ImmediatePriority,flushSyncCallbackQueue),error$592;}finally{isFlushingSyncQueue=!1}}}function resolveDefaultProps(Component,baseProps){if(Component&&Component.defaultProps){baseProps=_assign({},baseProps);Component=Component.defaultProps;for(var propName in Component)void 0===baseProps[propName]&&(baseProps[propName]=Component[propName]);
...
function(){var latestGetSnapshot=refs.getSnapshot,latestSetSnapshot=refs.setSnapshot;try{latestSetSnapshot(latestGetSnapshot(source._source));var lane=requestUpdateLane(fiber);root.mutableReadLanes|=lane&root.pendingLanes}catch(error$593){latestSetSnapshot(function(){throw error$593;})}});"function"!==typeof unsubscribe&&error$jscomp$0("Mutable source subscribe function must return an unsubscribe function.");return unsubscribe},[source,subscribe]);objectIs(prevGetSnapshot,getSnapshot)&&objectIs(prevSource,
...
ReactCurrentDispatcher$1.current=InvalidNestedHooksDispatcherOnUpdateInDEV;try{var currentState=queue.lastRenderedState,eagerState=pending(currentState,action);update.eagerReducer=pending;update.eagerState=eagerState;if(objectIs(eagerState,currentState))return}catch(error$594){}finally{ReactCurrentDispatcher$1.current=prevDispatcher}}if("undefined"!==typeof jest&&(warnIfNotScopedWithMatchingAct(fiber),executionContext===NoContext&&!1===IsSomeRendererActing.current&&!1===IsThisRendererActing.current)){queue=
